...
abstract class BasesfGuardUser extends BaseObject  implements Persistent {
	 ...
	 */
	protected $collAdvices;

	/**
	 * @var        Criteria The criteria used to select the current contents of collAdvices.
	 */
	private $lastAdviceCriteria = null;
	...
	public function reload($deep = false, PropelPDO $con = null)
	{
		...
		if ($deep) {  // also de-associate any related objects?
			...
			$this->collAdvices = null;
			$this->lastAdviceCriteria = null;
	...
	protected function doSave(PropelPDO $con)
	{
		...
		if (!$this->alreadyInSave) {
			...
			if ($this->collAdvices !== null) {
				foreach ($this->collAdvices as $referrerFK) {
					if (!$referrerFK->isDeleted()) {
						$affectedRows += $referrerFK->save($con);
					}
				}
	...
	public function copyInto($copyObj, $deepCopy = false)
	{
		...
		if ($deepCopy) {
			...
			foreach ($this->getAdvices() as $relObj) {
				if ($relObj !== $this) {  // ensure that we don't try to copy a reference to ourselves
					$copyObj->addAdvice($relObj->copy($deepCopy));
				}
			}
	 ...
	 * related Commentss from storage. If this sfGuardUser is new, it will return
	 ...
	 */
	public function getCommentssJoinAdvice($criteria = null, $con = null, $join_behavior = Criteria::LEFT_JOIN)
	{
		if ($criteria === null) {
			$criteria = new Criteria(sfGuardUserPeer::DATABASE_NAME);
		}
		...
		if ($this->collCommentss === null) {
			if ($this->isNew()) {
				...
				$this->collCommentss = CommentsPeer::doSelectJoinAdvice($criteria, $con, $join_behavior);
			}
		} else {
			// the following code is to determine if a new query is
			// called for.  If the criteria is the same as the last
	...
	/**
	 * Clears out the collAdvices collection (array).
	 *
	 * This does not modify the database; however, it will remove any associated objects, causing
	 * them to be refetched by subsequent calls to accessor method.
	 *
	...
	{
		$this->collAdvices = null; // important to set this to NULL since that means it is uninitialized
	}

	/**
	 * Initializes the collAdvices collection (array).
	 *
	 * By default this just sets the collAdvices collection to an empty array (like clearcollAdvices());
	 * however, you may wish to override this method in your stub class to provide setting appropriate
	 * to your application -- for example, setting the initial array to the values stored in database.
	 *
	 * @return     void
	 */
	public function initAdvices()
	{
		$this->collAdvices = array();
	}
	 ...
	 * If this collection has already been initialized with an identical Criteria, it returns the collection.
	 * Otherwise if this sfGuardUser has previously been saved, it will retrieve
	 * related Advices from storage. If this sfGuardUser is new, it will return
	 * an empty collection or the current collection, the criteria is ignored on a new object.
	 *
	 * @param      PropelPDO $con
	 * @param      Criteria $criteria
	 ...
	 * @throws     PropelException
	 */
	public function getAdvices($criteria = null, PropelPDO $con = null)
	{
		if ($criteria === null) {
			$criteria = new Criteria(sfGuardUserPeer::DATABASE_NAME);
		}
		...
		if ($this->collAdvices === null) {
			if ($this->isNew()) {
			   $this->collAdvices = array();
				...
				$criteria->add(AdvicePeer::USER_ID, $this->id);

				AdvicePeer::addSelectColumns($criteria);
				$this->collAdvices = AdvicePeer::doSelect($criteria, $con);
			}
		} else {
			// criteria has no effect for a new object
			if (!$this->isNew()) {
				...
				AdvicePeer::addSelectColumns($criteria);
				if (!isset($this->lastAdviceCriteria) || !$this->lastAdviceCriteria->equals($criteria)) {
					$this->collAdvices = AdvicePeer::doSelect($criteria, $con);
				}
			}
		}
		$this->lastAdviceCriteria = $criteria;
		return $this->collAdvices;
	}
	 ...
	 * @throws     PropelException
	 */
	public function countAdvices(Criteria $criteria = null, $distinct = false, PropelPDO $con = null)
	{
		if ($criteria === null) {
			$criteria = new Criteria(sfGuardUserPeer::DATABASE_NAME);
		} else {
	 ...
	 * @throws     PropelException
	 */
	public function addAdvice(Advice $l)
	{
		if ($this->collAdvices === null) {
			$this->initAdvices();
		}
		if (!in_array($l, $this->collAdvices, true)) { // only add it if the **same** object is not already associated
			array_push($this->collAdvices, $l);
			$l->setsfGuardUser($this);
		}
	}
	 ...
	 * Otherwise if this sfGuardUser is new, it will return
	 * an empty collection; or if this sfGuardUser has previously
	 * been saved, it will retrieve related Advices from storage.
	 *
	 * This method is protected by default in order to keep the public
	 * api reasonable.  You can provide public methods for those you
	 * actually need in sfGuardUser.
	 */
	public function getAdvicesJoinCategoriesRelatedByCategoryId($criteria = null, $con = null, $join_behavior = Criteria::LEFT_JOIN)
	{
		if ($criteria === null) {
			$criteria = new Criteria(sfGuardUserPeer::DATABASE_NAME);
		}
		...
		if ($this->collAdvices === null) {
			if ($this->isNew()) {
				...
				$this->collAdvices = AdvicePeer::doSelectJoinCategoriesRelatedByCategoryId($criteria, $con, $join_behavior);
			}
		} else {
			// the following code is to determine if a new query is
			// called for.  If the criteria is the same as the last
	 ...
	 * actually need in sfGuardUser.
	 */
	public function getAdvicesJoinCategoriesRelatedByCategoryUpcategoryId($criteria = null, $con = null, $join_behavior = Criteria::LEFT_JOIN)
	{
		if ($criteria === null) {
			$criteria = new Criteria(sfGuardUserPeer::DATABASE_NAME);
		}
		...
		if ($this->collAdvices === null) {
			if ($this->isNew()) {
				...
				$this->collAdvices = AdvicePeer::doSelectJoinCategoriesRelatedByCategoryUpcategoryId($criteria, $con, $join_behavior);
			}
		} else {
			// the following code is to determine if a new query is
			// called for.  If the criteria is the same as the last
	...
	public function clearAllReferences($deep = false)
	{
		if ($deep) {
			...
			if ($this->collAdvices) {
				foreach ((array) $this->collAdvices as $o) {
					$o->clearAllReferences($deep);
				}
			}
