...
abstract class BaseImages extends BaseObject  implements Persistent {
	...
	/**
	 * The value for the advice_id field.
	 * @var        int
	 */
	protected $advice_id;
	...
	/**
	 * Get the [advice_id] column value.
	 * 
	 * @return     int
	 */
	...
	public function setUrl($v)
	{
		...
		if ($this->url !== $v) {
			$this->url = $v;
			$this->modifiedColumns[] = ImagesPeer::URL;
		}
	...
	public function setText($v)
	{
		...
		if ($this->text !== $v) {
			$this->text = $v;
			$this->modifiedColumns[] = ImagesPeer::TEXT;
		}
	...
	/**
	 * Set the value of [advice_id] column.
	 * 
	 * @param      int $v new value
	 * @return     Images The current object (for fluent API support)
	 */
	public function setAdviceId($v)
	{
		if ($v !== null) {
			$v = (int) $v;
		}
		...
		if ($this->advice_id !== $v) {
			$this->advice_id = $v;
			$this->modifiedColumns[] = ImagesPeer::ADVICE_ID;
		}

		if ($this->aAdvice !== null && $this->aAdvice->getId() !== $v) {
			$this->aAdvice = null;
		}
	...
	public function hydrate($row, $startcol = 0, $rehydrate = false)
	{
		try {
			...
			$this->advice_id = ($row[$startcol + 3] !== null) ? (int) $row[$startcol + 3] : null;
			$this->resetModified();
			...
			return $startcol + 4; // 4 = ImagesPeer::NUM_COLUMNS - ImagesPeer::NUM_LAZY_LOAD_COLUMNS).
	...
	public function ensureConsistency()
	{
		...
		if ($this->aAdvice !== null && $this->advice_id !== $this->aAdvice->getId()) {
			$this->aAdvice = null;
		}
	} // ensureConsistency
	...
	public function delete(PropelPDO $con = null)
	{
		...
		try {
			...
			foreach (sfMixer::getCallables('BaseImages:delete:pre') as $callable)
			{
			...
			if ($ret) {
				ImagesPeer::doDelete($this, $con);
				$this->postDelete($con);
				// symfony_behaviors behavior
				foreach (sfMixer::getCallables('BaseImages:delete:post') as $callable)
				{
	...
	public function save(PropelPDO $con = null)
	{
		...
		try {
			...
			// symfony_behaviors behavior
			foreach (sfMixer::getCallables('BaseImages:save:pre') as $callable)
			{
			...
			if ($ret) {
				...
				$this->postSave($con);
				// symfony_behaviors behavior
				foreach (sfMixer::getCallables('BaseImages:save:post') as $callable)
				{
	...
	protected function doSave(PropelPDO $con)
	{
		...
		if (!$this->alreadyInSave) {
			...
			if ($this->isModified()) {
				if ($this->isNew()) {
					$pk = ImagesPeer::doInsert($this, $con);
					...
					$affectedRows += ImagesPeer::doUpdate($this, $con);
				}
	...
	protected function doValidate($columns = null)
	{
		if (!$this->alreadyInValidation) {
			...
			if ($this->aAdvice !== null) {
				if (!$this->aAdvice->validate($columns)) {
					$failureMap = array_merge($failureMap, $this->aAdvice->getValidationFailures());
				}
			}
	...
	public function fromArray($arr, $keyType = BasePeer::TYPE_PHPNAME)
	{
		...
		if (array_key_exists($keys[3], $arr)) $this->setAdviceId($arr[$keys[3]]);
	}
	...
	public function buildCriteria()
	{
		...
		if ($this->isColumnModified(ImagesPeer::URL)) $criteria->add(ImagesPeer::URL, $this->url);
		if ($this->isColumnModified(ImagesPeer::TEXT)) $criteria->add(ImagesPeer::TEXT, $this->text);
		if ($this->isColumnModified(ImagesPeer::ADVICE_ID)) $criteria->add(ImagesPeer::ADVICE_ID, $this->advice_id);
	...
	public function copyInto($copyObj, $deepCopy = false)
	{
		...
		$copyObj->setAdviceId($this->advice_id);
	...
	{
	  if (!$callable = sfMixer::getCallable('BaseImages:'.$method))
	  {
	    throw new sfException(sprintf('Call to undefined method BaseImages::%s', $method));
	  }
